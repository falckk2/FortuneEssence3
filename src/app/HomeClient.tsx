...
interface HomeClientProps {
  /** Featured products fetched server-side (FABLE-008); null if that fetch failed. */
...
export default function HomeClient({ initialProducts }: HomeClientProps) {
  ...
  // Client-side fallback only when the server-side fetch failed — keeps the
  // page functional if the DI container had an infrastructure hiccup.
  useEffect(() => {
