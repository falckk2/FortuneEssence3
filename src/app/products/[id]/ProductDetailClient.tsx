...
interface ProductDetailClientProps {
  productId: string;
  /** Product fetched server-side for SEO; the component falls back to a client fetch when null. */
...
export default function ProductDetailClient({ productId, initialProduct }: ProductDetailClientProps) {
  ...
  useEffect(() => {
    // Bundle configuration and related products always load client-side.
    const fetchSecondaryData = async (prod: Product) => {
      if (prod.category === 'bundles') {
      ...
      const relatedResponse = await fetch(`/api/products?category=${prod.category}&limit=4`);
    ...
    const fetchProductData = async () => {
      ...
      try {
        if (initialProduct) {
          // Product was server-rendered — skip re-fetching it.
          setProduct(initialProduct);
          await fetchSecondaryData(initialProduct);
          return;
        }
        ...
        if (productData.success && productData.data) {
          ...
          await fetchSecondaryData(productData.data);
        } else {
