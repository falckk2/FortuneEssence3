  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-12">
        <h2 className="text-2xl font-bold text-forest-900 dark:text-[#E8EDE8] mb-4">
          {locale === 'sv' ? 'Din varukorg är tom' : 'Your cart is empty'}
        </h2>
        <p className="text-forest-600 dark:text-[#B8C5B8] mb-6">
          {locale === 'sv'
    ...
    <div className="max-w-4xl mx-auto">
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          {/* Contact Information */}
          <div className="bg-white dark:bg-[#242a28] p-6 rounded-lg shadow-sm border border-cream-200 dark:border-[#3f4946]">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  {...register('email')}
                  type="email"
                  className="w-full px-3 py-2 border border-cream-300 dark:border-[#4a5552] rounded-md bg-white dark:bg-[#2a3330] dark:text-[#E8EDE8] dark:placeholder-[#6B7B6B] focus:outline-none focus:ring-2 focus:ring-sage-500 focus:border-transparent"
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <label className="flex items-center">
                <input
                  ...
                  type="checkbox"
                  className="rounded border-cream-300 dark:border-[#4a5552] text-sage-600 focus:ring-sage-500"
                />
                <span className="ml-2 text-sm text-forest-600 dark:text-[#B8C5B8]">
              ...
              <>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-sm rounded-md transition-colors ${
                      filterType === 'all'
                        ...
                        : 'bg-cream-100 dark:bg-[#343c39] text-forest-700 dark:text-[#C5D4C5] hover:bg-cream-200 dark:hover:bg-[#3f4946]'
                    }`}
                  >
                    {locale === 'sv' ? 'Alla' : 'All'}
                  </button>
                ...
                <div className="space-y-2">
                  {Object.entries(
                    ...
                    return (
                      <div key={carrierCode} className={`border rounded-lg overflow-hidden transition-all ${hasSelected ? 'border-sage-600' : 'border-cream-200 dark:border-[#3f4946]'}`}>
                        {/* Carrier header */}
                        <button
                          type="button"
                          onClick={() => setExpandedCarrier(isExpanded && !hasSelected ? null : carrierCode)}
                          className={`w-full flex items-center justify-between px-4 py-3 text-left transition-colors ${hasSelected ? 'bg-sage-50 dark:bg-sage-900/30' : 'hover:bg-cream-50 dark:hover:bg-[#2a3330]'}`}
                        >
                          <div className="flex items-center gap-3">
                            <span className="font-semibold text-sm text-forest-900 dark:text-[#E8EDE8]">{carrierName}</span>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-bold text-forest-900 dark:text-[#E8EDE8]">
                              {locale === 'sv' ? 'fr.' : 'from'} {cheapest === 0 ? (locale === 'sv' ? 'Gratis' : 'Free') : `${cheapest.toFixed(0)} kr`}
                            </span>
                            <ChevronDownIcon className={`w-4 h-4 text-forest-400 dark:text-[#6B7B6B] transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
                          </div>
                        </button>

                        {/* Expanded options */}
                        {isExpanded && (
                          <div className="border-t border-cream-100 dark:border-[#3f4946]">
                            {rates.map((rate) => (
                              <button
                                key={rate.id}
                                type="button"
                                ...
                                className={`w-full flex items-center justify-between px-4 py-2.5 text-left transition-colors ${
                                  selectedShipping?.id === rate.id
                                    ...
                                    : 'hover:bg-cream-50 dark:hover:bg-[#2a3330]'
                                }`}
                              >
                                <div className="flex items-center gap-2">
                                  <div className="flex-shrink-0">
                                    {selectedShipping?.id === rate.id ? (
                                      ...
                                      <div className="w-4 h-4 rounded-full border-2 border-cream-300 dark:border-[#4a5552]"></div>
                                    )}
                                  </div>
                                  <div>
                                    <span className="text-sm text-forest-900 dark:text-[#E8EDE8]">{rate.serviceType ? rate.name.replace(carrierName, '').trim() || rate.name : rate.name}</span>
                                    <span className="text-xs text-forest-500 dark:text-[#8A9A8A] ml-2">
                                ...
                                <span className={`text-sm font-semibold ${rate.price === 0 ? 'text-green-600' : 'text-forest-900 dark:text-[#E8EDE8]'}`}>
            ...
            {availablePaymentMethods.filter(m => m.enabled).length > 0 ? (
              <div className="grid grid-cols-2 gap-3">
                {availablePaymentMethods.filter(m => m.enabled).map((method) => {
                  ...
                  return (
                    <label key={method.id} className="flex items-center p-3 border border-cream-200 dark:border-[#3f4946] rounded-lg cursor-pointer hover:bg-cream-50 dark:hover:bg-[#2a3330]">
                      <input
                      ...
                      />
                      <IconComponent className="h-5 w-5 ml-3 text-forest-600 dark:text-[#B8C5B8]" />
                      <span className="ml-2 text-sm font-medium text-forest-900 dark:text-[#E8EDE8]">
            ...
            <div className="space-y-4">
              <label className="flex items-start">
                <input
                  ...
                  type="checkbox"
                  className="mt-1 rounded border-cream-300 dark:border-[#4a5552] text-sage-600 focus:ring-sage-500"
                />
                <span className="ml-2 text-sm text-forest-600 dark:text-[#B8C5B8]">
                  {locale === 'sv'
        ...
        <div className="lg:sticky lg:top-4 lg:h-fit">
          <div className="bg-white dark:bg-[#242a28] p-6 rounded-lg shadow-sm border border-cream-200 dark:border-[#3f4946]">
                ...
                <div className="space-y-4">
                  {items.map((item, i) => (
                    <div key={i} className="animate-pulse flex space-x-3">
                      <div className="rounded bg-cream-300 dark:bg-[#343c39] h-16 w-16"></div>
                      <div className="flex-1 space-y-2 py-1">
                        <div className="h-4 bg-cream-300 dark:bg-[#343c39] rounded w-3/4"></div>
                        <div className="h-3 bg-cream-300 dark:bg-[#343c39] rounded w-1/2"></div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                cartItems.map((item) => (
                  <div key={item.cartItemId || item.productId} className="flex gap-3">
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-forest-900 dark:text-[#E8EDE8] text-sm">
                        {getProductName(item.product)}
                      </p>
                      <p className="text-xs text-forest-600 dark:text-[#B8C5B8]">
                        ...
                        <div className="mt-1">
                          ...
                          <ul className="space-y-0.5">
                            {item.selectedProducts.map((selectedProduct, idx) => (
                              <li key={`${item.productId}-${selectedProduct.id}-${idx}`} className="flex items-center text-xs text-forest-600 dark:text-[#B8C5B8]">
            ...
            <div className="border-t border-cream-200 dark:border-[#3f4946] pt-4 space-y-2">
              <div className="flex justify-between text-sm text-forest-700 dark:text-[#C5D4C5]">
              ...
              <div className="border-t border-cream-200 dark:border-[#3f4946] pt-2 flex justify-between font-bold text-lg text-forest-900 dark:text-[#E8EDE8]">
            ...
            <button
              ...
              className="w-full mt-6 px-6 py-3 bg-sage-600 text-white font-medium rounded-lg hover:bg-sage-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isProcessing 
